...
class PlaywrightPhotoProvider(PlacePhotoProvider):
    """Retrieves place photos by automating a headless Chromium browser on Google Maps asynchronously.
    ...
    async def fetch_photo(self, place: PlaceItem) -> PlacePhoto | None:
        ...
        try:
            ...
            await maps_page.navigate(target_url)

            # Locate hero cover photo URL via Page Object Model
            img_src = await maps_page.locate_hero_photo_url()
            if not img_src:
                return None
            ...
            res = await page.request.get(high_res_url, timeout=self.timeout_ms)
            if not res.ok:
                # Fallback to original src if high_res_url fails
                res = await page.request.get(img_src, timeout=self.timeout_ms)
                if not res.ok:
                    return None

            content_type = res.headers.get("content-type", "image/jpeg")
            extension = "png" if "png" in content_type else "jpg"
            data_bytes = await res.body()
