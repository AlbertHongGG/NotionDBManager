...
class GoogleMapsPageObject:
    ...
    async def _find_hero_button(self) -> Any:
        ...
        try:
            btn = await self.page.wait_for_selector(self.PRIMARY_HERO_SELECTOR, timeout=min(self.timeout_ms, 5000))
            if btn:
                return btn
        except Exception:
            pass
    ...
    async def ensure_detail_view(self) -> None:
        ...
        try:
            feed_item = await self.page.wait_for_selector(self.SEARCH_FEED_ITEM_SELECTOR, timeout=2000)
            if feed_item:
    ...
    async def locate_hero_photo_url(self) -> str | None:
        ...
        hero_btn = await self._find_hero_button()
        if not hero_btn:
            # Maybe query landed on search results list; try entering first item
            await self.ensure_detail_view()
            hero_btn = await self._find_hero_button()
        ...
        try:
            img_elem = await hero_btn.wait_for_selector("img", timeout=5000)
            if not img_elem:
                return None
        except Exception:
            return None
